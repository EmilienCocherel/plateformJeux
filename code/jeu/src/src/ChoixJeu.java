...
public class ChoixJeu extends GridPane {
    ...
    ChoixJeu( ConnexionMySQL laConnexion){
	super();
	...
	if (laConnexion.estUnePartie(Integer.parseInt(numPartie.getText())))
		j=jeu.jouerCoup(Integer.parseInt(numPartie.getText()),Integer.parseInt(numJoueur.getText()),new Partage(this.laConnexion,this));
	else j=jeu.creerPartie(1,1,2, new Partage(this.laConnexion,this));
	this.add(j,0,3);
    }
}
